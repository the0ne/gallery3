(function($) {
   $.widget("ui.gallery_dialog",  {
     ...
     _show: function(sHref) {
       ...
       $("body").append(eDialog).gallery_show_loading("large");
      ...
      $.ajax({
         ...
         success: function(data, textStatus, xhr) {
           ...
           $("#g-dialog").html(content);
           $("body").gallery_show_loading("large");
       ...
       $("#g-dialog form").ajaxForm({
         ...
         success: function(data, textStatus, xhr) {
            var mimeType = /^(\w+\/\w+)\;?/.exec(xhr.getResponseHeader("Content-Type"));
             ...
             var content = "";
             if (mimeType == null || mimeType[1] != "application/json") {
               ...
               // but because its in an invisible iframe, it could be wrapped with html
               var matches = data.match("({.*})");
               if (matches) {           // json embedded in html
                 data = $.parseJSON(matches[0]);
               } else {
