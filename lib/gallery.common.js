(function ($) {
   ...
   */
  $.fn.gallery_show_loading = function(size) {
    return this.each(function(i){
      var loadingClass = "g-loading-" + size;
      if ($("." + loadingClass).length) {
        $("." + loadingClass).remove();
      } else {
        var viewportSize = $.gallery_get_viewport_size();
        $("<div class='" + loadingClass + "></div>")
          .appendTo(document.body);
        var top = (viewportSize.height() - $("." + loadingClass).height() - 2) / 2;
        var left = (viewportSize.width() - $("." + loadingClass).width() - 2) / 2;
        $("." + loadingClass).css({
          top: top + "px",
          left: left + "px",
          "z-index": 999
